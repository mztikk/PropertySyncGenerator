...
namespace PropertySyncGenerator
{
    internal class SyncMethod
    {
        ...
        private static Argument[] Arguments(string srcType, string targetType) => new Argument[] { new(srcType, "source"), new(targetType, "target") };
        ...
        private void Body(BodyWriter writer)
        {
            foreach (IPropertySymbol item in _targetProperties.Value)
            {
                ...
                {
                    writer.WriteLine($"{_arguments[1].Name}.{item.Name} = {_arguments[0].Name}.{_srcProperties.Value.First(x => x.Name == item.Name).Name};");
                }
            }
        }
